...
# 2021
setwd("C:/rstats/capacidades_ingles")
#setwd(path)
...
#getwd()
#save.image("capacities_raw_data.nov.21.RData")
...
### indice Firjan
link.firjan <- "https://www.firjan.com.br/data/files/11/04/E5/97/F1B8461049FF6646A8A809C2/Evolu__o%20do%20IFDM%20Geral%20-%202005%20a%202016.xlsx"

destfile <- "firjan.xlsx"
curl::curl_download(link.firjan, destfile)
ind.firjan <- read_excel(destfile)

ind.firjan <- ind.firjan %>%  
  janitor::row_to_names(row_number = 2)

names(ind.firjan)[c(5:length(names(ind.firjan)))]
...
```{r}
new.names <- c(names(ind.firjan)[c(1:4)], paste0(names(ind.firjan)[c(5:length(names(ind.firjan)))],
      "_",
      rep(2005:2016, each = 2)))

names(ind.firjan) <- new.names

ind.firjan <- ind.firjan %>%
  janitor::clean_names()


ind.firjan.2010 <- ind.firjan %>% 
  select(codigo, nota_2010) %>% 
  mutate(nota_2010 = as.double(nota_2010))
```
...
transp <- transp %>% #glimpse()
    select(uf, municipio, nota = nota_2a_avaliacao) %>%
...
dlookr::diagnose_category(capacities, faixa_pop) %>% 
  as.data.frame()

diagnose_paged_report(capacities)
```
...
```{r}
firjan.and.idhm <- capacities_2 %>% 
  select("codigo" = "cod_mun", idhm) %>% 
  mutate(codigo.2 = str_sub(codigo, 1, 6)) %>% 
  left_join(ind.firjan.2010, by = c("codigo.2" = "codigo")) %>% 
  drop_na()

cor(firjan.and.idhm$idhm, firjan.and.idhm$nota_2010)

firjan.and.idhm %>% 
  mutate(dif = idhm - nota_2010) %>% 
  arrange(desc(dif))

corr <- firjan.and.idhm %>% 
  select(2:3)
...
# setwd("C:/r_files/my_academic_projects/capacidades/capacitties")
#save.image("capacities_raw_data_3.01.dez.21.RData")
#load("capacities_raw_data_3.RData")
#load("capacities_raw_data_3.01.dez.21.RData")
```
...
# Adicionar Indice Firjan
```{r}
capacities_4 <- capacities_4 %>% 
  left_join(
    mutate(select(firjan.and.idhm, codigo, "nota.firjan.2010"= nota_2010 ), 
...
cap_cor <- capacities_4 %>% 
  select(pib_per_cap, idhm, nota.firjan.2010, starts_with("Num"), nota_transp, populacao)
...
leg <- leg %>% 
  ...
  select(id_municipio, maioria.leg)

capacities_5 <- capacities_4 %>% 
  left_join(
          mutate(leg, id_municipio = as.character(id_municipio)), 
          by = c("cod_mun" = "id_municipio"))
```
...
corr.2 <- round(cor(cap_cor.2), 2)

ggcorrplot(corr.2, hc.order = F, 
           type = "lower", 
           show.diag = T,
           pch = 12,
           pch.cex = 12,
...
olr_brasil <- vglm(idhm_quintil ~ log(pib_per_cap) +
                  Num_pl + Num_chl + Num_consor + nota_transp + maioria.leg,   
                  data = capacities_5, family = cumulative(parallel = T, reverse = T))
...
# 
```{r}
save.image("Capacities.01.dez.21._line 1043.RData")
```
